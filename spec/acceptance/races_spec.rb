...
resource "Races" do
  ...
  before (:each) do
    ...
    ActiveRecord::Base.connection.insert("INSERT INTO candidates (election_yr, can_id, can_nam, can_off, can_off_sta, can_off_dis, can_par_aff, can_inc_cha_ope_sea) VALUES (2014, 'S4NH00104', 'TESTERMAN, KAREN', 'S','NH', '00','REP','CHALLENGER')");

    # Races
    ActiveRecord::Base.connection.insert("INSERT INTO races (election_yr, can_id, on_ballot) VALUES (2014, 'H6NH01230', 1)");
    ActiveRecord::Base.connection.insert("INSERT INTO races (election_yr, can_id, on_ballot) VALUES (2014, 'H0NH02181', 1)");
    ActiveRecord::Base.connection.insert("INSERT INTO races (election_yr, can_id, on_ballot) VALUES (2014, 'S4NH00104', 0)");

  end

  after (:each) do
    ActiveRecord::Base.connection.delete("DELETE FROM candidates");
    ActiveRecord::Base.connection.delete("DELETE FROM races");
  end
  ...
  get "/races/:cycle/candidates/:id" do
    ...
    example "Getting a single house candidate" do
      ...
      expect(response_body).to be_json_eql({
        ...
        :base_uri => "http://example.org/races/2014/",
        :cycle => 2014,
        :results => [
          {
            :id => "H6NH01230",
  ...
  get "/races/:cycle/seats/:state" do
    let(:cycle) { "2014" }
    let(:state) { "NH" }
  ...
  get "/races/:cycle/seats/:state/:office" do
    let(:cycle) { "2014" }
    let(:state) { "NH" }
